...
func TestGetPwdTemplate(t *testing.T) {
	...
	template, terr := taskrun.GetPwdTemplate(path)
	if terr != nil {
...
func TestGetTemplate(t *testing.T) {
	...
	template, _, exists, _ := taskrun.GetTemplate()
	if !exists {
...
func TestGetVarImport(t *testing.T) {
	caseRunner("10", t, func(t *testing.T) {
		template, path, exists, _ := taskrun.GetTemplate()
		if !exists {
		...
		taskrun.RunTargets("script", true)
		test1Result := taskrun.GetPH("RUN.script.LOG.LAST")
		if test1Result == "" {
