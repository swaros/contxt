...
func (t *targetExecuter) executeTemplate(runAsync bool, runCfg configure.RunConfig, target string, scopeVars map[string]string) int {
	...
	if len(runCfg.Task) > 0 {
		...
		for curTIndex, script := range taskList {
			if strings.EqualFold(target, script.ID) {
					...
					if runAsync {
							...
							if t.watch.TaskRunsAtLeast(needTarget, 1) {
								...
								needExecs = append(needExecs, awaitgroup.FutureStack{
									AwaitFunc: func(ctx context.Context) interface{} {
										...
										return t.executeTemplate(true, runCfg, argNeed, argmap)
									},
									Argument: needTarget})
							}
						}
						...
						for _, needTarget := range script.Needs {
							if t.watch.TaskRunsAtLeast(needTarget, 1) { // do not run needs the already runs
								...
								t.executeTemplate(false, runCfg, needTarget, argmap)
							}
						}
					}
				...
				runTargetfutures := t.generateFuturesByTargetListAndExec(script.RunTargets, runCfg)
				...
				abort, returnCode, _ = t.TryParse(script.Script, func(codeLine string) (bool, int) {
					lineAbort, lineExitCode := t.lineExecuter()
				...
				nextfutures := t.generateFuturesByTargetListAndExec(script.Next, runCfg)
	...
	for _, needTarget := range RunTargets {
		...
		runTargetExecs = append(runTargetExecs, awaitgroup.FutureStack{
			AwaitFunc: func(ctx context.Context) interface{} {
				...
				return t.executeTemplate(true, runCfg, argTask, argmap)
			},
			Argument: needTarget})
